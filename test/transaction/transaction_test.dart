...
main() {
  ...
  test('can perform serialization', () {
    var testTransaction = testBuilder.build(true);
    expect(testTransaction.inputs[0].script?.toHex(),
        equals(testScriptHex)); //FIXME: SVScript does not properly process these human-readable script translations right now
  ...
  test('returns the fee as value of unspent output', () {
    expect(testBuilder.getFee(), equals(BigInt.from(10000)));
  });
  ...
  test('can serialize to a P2PKH transaction by default', () {
    ...
    var unlocker = P2PKHUnlockBuilder(privateKey.publicKey);
    var builder = TransactionBuilder()
        .spendFromOutpointWithSigner(signer, outpoint, TransactionInput.MAX_SEQ_NUMBER, unlocker) //set global sequenceNumber/nLocktime time for each Input created
  ...
  test('can calculate output amounts and correct change address', () {
    ...
    expect(BigInt.from(50000), equals(transaction.outputs[0].satoshis));
  ...
  test('transaction creation/serialization test vectors', () async {
    await File("${Directory.current.path}/test/data/tx_creation.json")
      ...
      List.from(jsonData).forEach((item) {
        ...
        var builder = TransactionBuilder()
            .spendFromUtxoMapWithSigner(signer, utxoMap, P2PKHUnlockBuilder(privKey.publicKey))
            .withFeePerKb(100000);
      ...
      await File("${Directory.current.path}/test/data/bip69.json")
        ...
        HashMap.from(jsonData)["inputs"].forEach((vector) {
          ...
          var txInputs = inputSet.map((input) {
            return TransactionInput(input["txId"], input["vout"], TransactionInput.MAX_SEQ_NUMBER, SVScript());
          }).toList();
