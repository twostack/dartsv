...
main() {
    test('should be able to compute sighash for a coinbase tx', () {
        ...
        var sighash = Sighash(tx).hash(SighashType.SIGHASH_ALL, 0, SVScript.fromString(""), BigInt.zero);
    ...
    test('Should require amount for sigHash ForkId=0', () {
        ...
        expect(() => Sighash(tx).hash(nhashtype, nin, subscript, null).toString(), throwsException);
    });
        ...
        await File("${Directory.current.path}/test/data/sighash.json")
            ...
            List.from(jsonData).forEach((vector) {
                ...
                if (vector.length != 1) {
                    ...
                    // sighash ought to be correct
                    expect(Sighash(tx).hash(nhashtype, nin, subscript, BigInt.zero), equals(sighashbuf));
                }
